...
public class TestCase3 {
	...
	void payControllerTest() throws Exception {
		...
		PaymentRequest paymentRequest = PaymentRequest
				...
				.validityPeriod("0522").cvc("527").price(20000).build();
		...
		//두번째 테스트
		CancelRequest cancelRequest = CancelRequest.builder().price(10000).vat("1000").managementNumber(pr.getString("managementNumber")).build();
		...
		Thread.sleep(1000);
		//네번째 테스트
		cancelRequest = CancelRequest.builder().price(10000).managementNumber(managementNumber).build();
		result = mvc.perform(delete("/payment/cancel").content(objectMapper.writeValueAsBytes(cancelRequest)).contentType(MediaType.APPLICATION_JSON))
				.andDo(print()).andReturn();
		content = result.getResponse().getContentAsString();
		rr = new JSONObject(content);
